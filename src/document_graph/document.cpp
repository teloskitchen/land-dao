...
namespace hashed
{
    ...
    void Document::modify()
    {
        ...
        eosio::checksum256 oldHash = hash;
        hashContents();

        document_table d_t(getContract(), getContract().value);
        auto hash_index = d_t.get_index<eosio::name("idhash")>();
        auto h_itr = hash_index.find(oldHash);

        EOS_CHECK(h_itr != hash_index.end(), "document does not exists: " + readableHash(oldHash));
        EOS_CHECK(hash_index.find(hash) == hash_index.end(), "document exists already: " + readableHash(hash));

        require_auth(h_itr->creator);

        hash_index.modify(h_itr, getContract(), [&](auto &d) {
            d = *this;
        });
    }
