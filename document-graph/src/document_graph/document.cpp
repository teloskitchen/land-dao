...
namespace hashed
{
    ...
    void Document::emplace()
    {
        ...
        // TODO: create a document subclass that enforces hashing and uniqueness
        // if this content exists already, error out and send back the hash of the existing document
        // EOS_CHECK(d_itr == hash_index.end(), "document exists already: " + readableHash(hash));
    ...
    void Document::modify()
    {
        ...
        hash_index.modify(d_itr, getContract(), [&](auto &d) {
            updated_date = eosio::current_time_point();
            d = *this;
        });
    }
