...
namespace hashed
{
    ...
    Edge::Edge(const eosio::name &contract,
               ...
               const uint64_t &from_node,
               const uint64_t &to_node,
    ...
    void Edge::write(const eosio::name &_contract,
                     ...
                     const uint64_t &_from_node,
                     const uint64_t &_to_node,
                     const eosio::name &_edge_name)
    {
        edge_table e_t(_contract, _contract.value);

        const int64_t edgeID = concatID(_from_node, _to_node, _edge_name);
        ...
        e_t.emplace(_contract, [&](auto &e) {
            e.id = edgeID;
            e.from_node_edge_name_index = concatID(_from_node, _edge_name);
            e.from_node_to_node_index = concatID(_from_node, _to_node);
            e.to_node_edge_name_index = concatID(_to_node, _edge_name);
            e.creator = _creator;
    ...
    {
        edge_table e_t(_contract, _contract.value);
        auto itr = e_t.find(concatID(_from_node, _to_node, _edge_name));
                   ...
                   const eosio::name &_edge_name)
    {
        ...
        EOS_CHECK(itr != e_t.end(), "edge does not exist: from " + std::to_string(_from_node) + " to " + std::to_string(_to_node) + " with edge name of " + _edge_name.to_string());
                   ...
                   const eosio::name &_edge_name)
    {
        ...
        auto index = concatID(_from_node, _edge_name);
        auto itr = fromEdgeIndex.find(index);

        EOS_CHECK(itr != fromEdgeIndex.end() && itr->from_node_edge_name_index == index, "edge does not exist: from " + std::to_string(_from_node) + " with edge name of " + _edge_name.to_string());
                     ...
                     const eosio::name &_edge_name)
    {
        ...
        auto index = concatID(_to_node, _edge_name);
        auto itr = toEdgeIndex.find(index);

        EOS_CHECK(itr != toEdgeIndex.end() && itr->to_node_edge_name_index == index, "edge does not exist: to " + std::to_string(_to_node) + " with edge name of " + _edge_name.to_string());
    ...
    void Edge::emplace()
    {
        ...
        id = concatID(from_node, to_node, edge_name);

        from_node_edge_name_index = concatID(from_node, edge_name);
        from_node_to_node_index = concatID(from_node, to_node);
        to_node_edge_name_index = concatID(to_node, edge_name);
    ...
    void Edge::erase()
    {
        ...
        EOS_CHECK(itr != e_t.end(), "edge does not exist: from " + std::to_string(from_node) + " to " + std::to_string(to_node) + " with edge name of " + edge_name.to_string());
        e_t.erase(itr);
    }
