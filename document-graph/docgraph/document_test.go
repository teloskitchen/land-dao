	...
	t.Run("Test Documents", func(t *testing.T) {
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				...
				lastDoc, err := docgraph.NewDocumentFromString(env.ctx, &env.api, env.Docs, env.Creators[0], string(data))
				assert.NilError(t, err)

				// unmarshal JSON into a Document
				newdata, err := ioutil.ReadFile(test.input)
				assert.NilError(t, err)
				var documentFromFile docgraph.Document
				err = json.Unmarshal(newdata, &documentFromFile)
				assert.NilError(t, err)
...
func TestUpdateDocument(t *testing.T) {
	...
	docId := loadedDoc.ID
	oldHash := loadedDoc.Hash

	// Act
	updatedDoc, err := docgraph.UpdateDocument(env.ctx, &env.api, env.Docs, loadedDoc, "examples/each-type.json")

	// Assert
	assert.NilError(t, err)
	assert.Assert(t, docId == updatedDoc.ID)
	assert.Assert(t, oldHash.String() != updatedDoc.Hash.String())

	data, err := ioutil.ReadFile("examples/each-type.json")
	assert.NilError(t, err)
	var documentFromFile docgraph.Document
	err = json.Unmarshal(data, &documentFromFile)
	assert.NilError(t, err)
	...
	// compare document from chain to document from file
	assert.Assert(t, updatedDoc.IsEqual(documentFromFile))
