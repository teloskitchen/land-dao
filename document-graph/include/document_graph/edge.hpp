...
namespace hashed
{
    ...
    {
        Edge();
        Edge(const eosio::name &contract, const eosio::name &creator, const uint64_t &fromNode,
             const uint64_t &toNode, const eosio::name &edgeName);
        ~Edge();

        void emplace();
        void erase();

        const uint64_t &getFromNode() { return from_node; }
        const uint64_t &getToNode() { return to_node; }
        ...
        // All of these indexes is expensive RAM-- how do we optimize in future release? 
