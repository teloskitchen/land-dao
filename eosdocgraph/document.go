...
func NewDocumentFromString(ctx context.Context, api *eos.API,
	contract, creator eos.AccountName, contentGroupsJson string) (Document, error) {

	var dump map[string]interface{}
	err := json.Unmarshal([]byte(contentGroupsJson), &dump)
	if err != nil {
		return Document{}, fmt.Errorf("cannot marshal to map[string]interface: %v", err)
	}

	dump["creator"] = creator

	actionBinary, err := api.ABIJSONToBin(ctx, contract, eos.Name("create"), dump)
	if err != nil {
		return Document{}, fmt.Errorf("api json to bin: %v", err)
	}

	actions := []*eos.Action{
		{
			Account: contract,
			Name:    eos.ActionName("create"),
	...
	if err != nil {
		return Document{}, fmt.Errorf("cannot execute transaction: %v", err)
	}

	lastDoc, err := GetLastDocument(ctx, api, contract)
	if err != nil {
		return Document{}, fmt.Errorf("cannot get last document: %v", err)
	}
	return lastDoc, nil
}
...
func updateDocumentTrx(ctx context.Context, api *eos.API,
	contract eos.AccountName, document Document, fileName string) (Document, error) {
	...
	dump["document_hash"] = document.Hash
	...
	actions := []*eos.Action{
		{
			...
			Name:    action,
			Authorization: []eos.PermissionLevel{
				{Actor: document.Creator, Permission: eos.PN("active")},
			},
			ActionData: eos.NewActionDataFromHexData([]byte(actionBinary)),
		}}
	...
	updatedDocument, err := LoadDocumentByID(ctx, api, contract, document.ID)
	if err != nil {
		return Document{}, fmt.Errorf("can not load updated document %v: %v", fileName, err)
	}

	return updatedDocument, nil
}

func UpdateDocument(ctx context.Context, api *eos.API,
	contract eos.AccountName, document Document, fileName string) (Document, error) {

	return updateDocumentTrx(ctx, api, contract, document, fileName)
}
...
func LoadDocumentByID(ctx context.Context, api *eos.API,
	contract eos.AccountName,
	id uint64) (Document, error) {
	...
	request.Scope = string(contract)
	request.Table = "documents"
	request.LowerBound = fmt.Sprint(id)
	request.UpperBound = fmt.Sprint(id)
	request.Limit = 1
	request.JSON = true
	response, err := api.GetTableRows(ctx, request)
	if err != nil {
		return Document{}, fmt.Errorf("get table rows %v: %v", id, err)
	}

	err = response.JSONToStructs(&documents)
	if err != nil {
		return Document{}, fmt.Errorf("json to structs %v: %v", id, err)
	}

	if len(documents) == 0 {
		return Document{}, fmt.Errorf("document not found %v: %v", id, err)
	}
	return documents[0], nil
}
